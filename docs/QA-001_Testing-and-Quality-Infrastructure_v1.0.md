...
- repository data version 0.11.0;
...
## Sprint 11 scalable-ingestion coverage

Application and repository contract v0.11.0 extend the permanent quality system with:

- canonical RC contract and legacy-adapter unit tests;
- conforming RC-006 integration proof without compiler source changes;
- malformed-record preflight and no-publication regression coverage;
- transactional publisher verification;
- dynamic repository-count and source-provenance closure checks;
- synthetic 20-, 25- and 150-record scale validation covering identifiers, search, graph inventory, manifest generation, output size and runtime;
- CI gates for Reference Standard preflight and compiler scale validation.

The five frozen public records remain the current corpus, while quality checks now validate the machinery required to expand safely.
