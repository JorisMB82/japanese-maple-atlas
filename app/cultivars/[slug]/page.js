...
export default function CultivarPage({params}){
 ...
 return <>
  ...
  <section className="profileHero mediaProfileHero">
   <div><div className="profileMeta"><span className="referenceId">{c.id}</span><StatusBadge status={c.status}/></div><p className="speciesName"><em>{c.species}</em></p><h1>{c.cultivar}</h1><p className="scientificName"><em>{c.scientificName}</em></p><p className="lead">{c.summary}</p><div className="heroActions"><a className="button" href={`/compare?a=${c.slug}`}>Compare cultivar</a><a className="button secondary" href={`/graph?node=${c.id}`}>Open graph node</a><a className="button secondary" href="#evidence">Inspect evidence</a></div></div>
   <MediaPlate media={c.primaryMedia} cultivar={c}/>
  </section>
  <div className="notice"><strong>Canonical source:</strong> this profile is compiled from the frozen {c.referenceStandard.id} Reference Standard and protected by SHA-256. Graph relationships are separately governed, evidence-linked comparisons and do not authenticate a specimen.</div>
  ...
  <section className="profileSection twoColumn"><article><div className="kicker">Repository identity</div><h2>Identity and classification</h2><p>{c.sections.identity}</p><dl className="traitDl"><dt>Accepted taxon</dt><dd><em>{c.taxon.scientificName}</em></dd><dt>Taxonomic graph edge</dt><dd>{c.taxonomicRelationships[0]?.relativeLabel}</dd></dl></article><article><div className="kicker">Historical record</div><h2>History</h2><p>{c.sections.history}</p></article></section>
  ...
  <section className="profileSection"><div className="sectionHeading"><div><div className="kicker">Knowledge graph</div><h2>Governed cultivar relationships</h2></div><p>{c.graphRelationships.length} explicit links · <a href={`/graph?node=${c.id}`}>explore node</a></p></div><RelationshipPanel relationships={c.graphRelationships}/></section>
