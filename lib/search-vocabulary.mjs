...
export const SEMANTIC_CONCEPTS = Object.freeze([
  ...
  },
  {
    ...
    fields: ['leafForm'],
    matchTerms: ['dissected', 'laceleaf', 'lace-like', 'narrow-lobed'],
    excludeTerms: ['non-dissected', 'non dissected', 'not a dissected', 'not dissected']
  },
  {
...
function conceptMatchesField(concept, value) {
  const haystack = normalise(Array.isArray(value) ? value.join(' ') : value);
  if (!haystack) return false;
  if ((concept.excludeTerms || []).some(term => haystack.includes(normalise(term)))) return false;
...
export function conceptsForDocument(fields) {
  return SEMANTIC_CONCEPTS.filter(concept => concept.fields.some(field => conceptMatchesField(concept, fields[field])));
}
