...
function hydrateCatalogueCultivar(cultivar) {
  ...
  const media = Array.isArray(cultivar.media) ? cultivar.media : [];
  const primaryMedia = cultivar.primaryMedia || media.find(item => item.isPrimary) || null;
  return {
    ...cultivar,
...
export function getMediaForCultivar(cultivarId) {
  if (/^CUL-/.test(cultivarId || '')) return allCultivars.find(item => item.cultivarId === cultivarId)?.media || [];
  return referenceRepository.getMediaForCultivar(cultivarId);
}
export function getPrimaryMediaForCultivar(cultivarId) {
  const media = getMediaForCultivar(cultivarId);
  return media.find(item => item.isPrimary || item.role === 'identity-plate') || media[0];
}
