...
export function validateCatalogueMediaSidecar(sidecar, schema) {
  ...
  const primary = assets.filter(asset => asset.isPrimary);
  const roles = assets.map(asset => asset.role);

  if (!unique(ids)) errors.push(`${sidecar?.cultivarId || 'unknown'}: media IDs must be unique`);
  if (assets.length > 0 && primary.length !== 1) errors.push(`${sidecar.cultivarId}: approved gallery requires exactly one primary item`);
  if (primary[0] && primary[0].role !== 'habit-primary') errors.push(`${sidecar.cultivarId}: primary item must use habit-primary role`);

  for (const asset of assets) {
    if (asset.cultivarId !== sidecar.cultivarId) errors.push(`${asset.id}: cultivarId must match sidecar`);
    if (!asset.id.startsWith(`MED-${sidecar.cultivarId}-`)) errors.push(`${asset.id}: media ID must be scoped to ${sidecar.cultivarId}`);
    if (asset.rightsBasis === 'creative-commons' && !asset.licenceUrl) errors.push(`${asset.id}: Creative Commons item requires licenceUrl`);
    if (!nonEmpty(asset.attributionText)) errors.push(`${asset.id}: attributionText is required`);
    if (asset.identity?.confidence !== 'documented' && !/not independently/i.test(identityQualification(asset))) {
      errors.push(`${asset.id}: non-documented identity requires conspicuous independent-authentication qualification`);
    }
    if (asset.privacy?.gpsRetained !== false || asset.privacy?.exifRetained !== false) errors.push(`${asset.id}: public derivatives must remove GPS and EXIF metadata`);
    const derivativeProfiles = (asset.derivatives || []).map(item => item.profile);
    for (const required of ['thumb', 'card', 'display', 'archive']) {
      if (derivativeProfiles.filter(profile => profile === required).length !== 1) errors.push(`${asset.id}: derivative profile ${required} must appear exactly once`);
    }
  }
  ...
  if (sidecar?.status === 'approved' && assets.length === 0) errors.push(`${sidecar.cultivarId}: approved sidecar requires at least one lawful visual item`);
  if (sidecar?.status === 'approved' && needsException && !sidecar.roleException) {
    errors.push(`${sidecar.cultivarId}: fewer-than-three or missing-role gallery requires roleException`);
  }
  if (sidecar?.roleException) {
    const declared = [...sidecar.roleException.missingRoles].sort();
    const actual = [...missingRoles].sort();
    if (JSON.stringify(declared) !== JSON.stringify(actual)) errors.push(`${sidecar.cultivarId}: roleException missingRoles must match actual missing required roles`);
  }
  if (!needsException && sidecar?.roleException) errors.push(`${sidecar.cultivarId}: complete gallery cannot retain a roleException`);
