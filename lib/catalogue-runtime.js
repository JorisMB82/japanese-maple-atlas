...
function attachMedia(record) {
  ...
  const declaredPrimaryId = record.mediaIds?.[0] || null;
  const sidecarPrimary = sidecar?.assets?.find(asset => asset.isPrimary) || null;
  const linked = record.mediaState === 'approved-primary'
    && Boolean(declaredPrimaryId)
    && sidecarPrimary?.id === declaredPrimaryId;
  const media = linked ? sidecar.assets : [];
  const primaryMedia = linked ? sidecarPrimary : null;
  return {
    ...record,
    media,
    primaryMedia,
    mediaState: linked ? 'approved-gallery' : record.mediaState,
    mediaGovernedGap: linked ? null : record.mediaGovernedGap,
    visualComplete: linked && !sidecar.roleException,
