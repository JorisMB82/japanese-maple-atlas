...
export function searchCultivars(filters = {}){
  const filtered = filterCultivars(getCultivars(), filters);
  return sortCultivars(filtered, filters.sort || 'relevance');
}
...
export function getIntegrityReport(){
 ...
 return {
   ...
   issues: missing,
   checkedObjects: Object.values(manifest.objectCounts).reduce((a,b)=>a+b,0)
 };
}
