...
const indexes = { cultivars: byId(cultivars), assertions: byId(assertions), evidence: byId(evidence), sources: byId(sources), taxa: byId(taxa), relationships: byId(relationships), media: byId(media) };
...
export function hydrateCultivar(cultivar){
  ...
  const hydratedMedia = cultivar.mediaIds.map(getMedia).filter(Boolean);
  return { ...cultivar, species: taxon?.scientificName || cultivar.scientificName.split(" '")[0], taxon, assertions: hydratedAssertions, relationships: hydratedRelationships, media: hydratedMedia };
}
...
export function getIntegrityReport(){
 ...
 for(const c of cultivars){
  ...
  for(const id of c.mediaIds) if(!indexes.media.has(id)) missing.push(`${c.id}: missing media ${id}`);
 }
